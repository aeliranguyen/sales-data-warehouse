...
WITH source AS (
    ...
    SELECT
        ...
        CAST(
            NULLIF(TRIM(expected_delivery_date), '')
            AS DATE
        ) AS expected_delivery_date,

        CAST(
            NULLIF(TRIM(actual_delivery_date), '')
            AS DATE
        ) AS actual_delivery_date,

        NULLIF(TRIM(ontime_delivery), '') AS ontime_delivery,
    ...
    WHERE order_id IS NOT NULL
      AND LOWER(TRIM(order_id)) NOT IN ('', 'null', 'none', 'nan')
